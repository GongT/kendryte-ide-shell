...
export class Mi2AutomaticResponder {
	...
	constructor(
		...
		this.controller = new Mi2CommandController(process.stdin, process.stdout);
	...
	public waitContinue(): Promise<void> {
		...
		if (!this.awaitingContinue) {
			...
			to.then(() => {
				this.awaitingContinue.error(new Error('cannot continue program in 5s'));
			}).catch(() => undefined);
		}
